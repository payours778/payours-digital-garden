  ...
  return (
    <div className="rounded-xl surface-strong backdrop-blur-xl border border-subtle">
      <div className="px-4 py-3 border-b border-subtle flex items-center gap-2">
        ...
        <span className="text-xs font-medium text-secondary">日历</span>
      ...
      <div className="p-3">
        ...
        <div className="grid grid-cols-7 gap-0.5">
          ...
          {days.map((day) => {
            ...
            if (isToday) {
              cls += " bg-slate-200 dark:bg-slate-600 text-primary font-medium";
            } else if (isWeekend) {
              cls += " text-tertiary hover:surface-card-hover";
            } else {
              cls += " text-secondary hover:surface-card-hover";
            }
            ...
            return (
              <div key={day} className={cls}>
                {day}
                {hasActivity && !isToday && (
                  <span className="absolute -bottom-px left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-tertiary" />
                )}
              </div>
            );
          })}
