...
export default function FarmGamePage() {
  ...
  const saveState = useCallback(async (state: Partial<{
    ...
    plots: Plot[]; inventory: InventoryItem[];
    seedInventory: InventoryItem[]; itemInventory: Item[];
    activeBuffs: ActiveBuff[]; growthBoostMultiplier: number;
    refreshCount: number;
  }>) => {
    if (!user) return;
    try {
      await apiPut("/api/games/farm/state", {
        coins: state.coins ?? coins,
        level: state.level ?? level,
        exp: state.exp ?? exp,
        plots: state.plots ?? plots,
        inventory: state.inventory ?? inventory,
        seedInventory: state.seedInventory ?? seedInventory,
        itemInventory: state.itemInventory ?? itemInventory,
        activeBuffs: state.activeBuffs ?? activeBuffs,
        growthBoostMultiplier: state.growthBoostMultiplier ?? growthBoostMultiplier,
        refreshCount: state.refreshCount ?? refreshCount,
      });
    } catch (err) {
      // 静默失败
    }
  }, [user, coins, level, exp, plots, inventory, seedInventory, itemInventory, activeBuffs, growthBoostMultiplier, refreshCount]);

  // 使用 ref 始终引用最新的 saveState，避免定时器闭包过期导致旧数据覆盖
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await apiGet("/api/games/farm/state");
        setCoins(data.coins ?? 100);
        setLevel(data.level ?? 1);
        setExp(data.exp ?? 0);
        setPlots(data.plots?.length ? data.plots : INITIAL_PLOTS);
        ...
        setItemInventory(data.itemInventory ?? []);
        setActiveBuffs(data.activeBuffs ?? []);
        setGrowthBoostMultiplier(data.growthBoostMultiplier ?? 1);
    ...
    const interval = setInterval(() => {
      ...
      setActiveBuffs(prev => {
        ...
        if (stillActive.length !== prev.length) {
          const newMultiplier = stillActive.some(b => b.effect === "growth_boost") ? 2 : 1;
          setGrowthBoostMultiplier(newMultiplier);
          // buff 过期后保存最新状态（使用 ref 避免闭包过期）
          saveStateRef.current({ activeBuffs: stillActive, growthBoostMultiplier: newMultiplier });
        }
        return stillActive;
      });
    }, 500);
  ...
  const addExp = (amount: number): { exp: number; level: number } => {
    const newExp = exp + amount;
    const needed = expForLevel(level);
    if (newExp >= needed) {
      const newLevel = level + 1;
      const remainingExp = newExp - needed;
      setLevel(newLevel);
      setExp(remainingExp);
      saveState({ level: newLevel, exp: remainingExp });
      showNotification(`🎉 升级了！现在是 ${newLevel} 级！`);
      return { exp: remainingExp, level: newLevel };
    } else {
      setExp(newExp);
      saveState({ exp: newExp });
      return { exp: newExp, level };
    }
  };
  ...
  const plantCrop = (plotId: number) => {
    ...
    const newSeedInventory = seedInventory
      .map((item) =>
        item.cropId === selectedCropId ? { ...item, count: item.count - 1 } : item
      )
      .filter((item) => item.count > 0);
    setSeedInventory(newSeedInventory);

    const newPlots = plots.map((p) =>
      p.id === plotId
        ? { ...p, cropId: selectedCropId, plantedAt: Date.now(), stage: "seed" as const }
        : p
    );
    setPlots(newPlots);

    saveState({ seedInventory: newSeedInventory, plots: newPlots });
  ...
  const buySeed = (cropId: string) => {
    ...
    const newSeedInventory = (() => {
      const existing = seedInventory.find((item) => item.cropId === cropId);
      if (existing) {
        return seedInventory.map((item) =>
          item.cropId === cropId ? { ...item, count: item.count + 1 } : item
        );
      }
      return [...seedInventory, { cropId, count: 1 }];
    })();
    setSeedInventory(newSeedInventory);

    saveState({ coins: newCoins, seedInventory: newSeedInventory });
  ...
  const harvestCrop = (plotId: number) => {
    ...
    const newPlots = plots.map((p) =>
      p.id === plotId ? { ...p, cropId: null, plantedAt: null, stage: "empty" as const } : p
    );
    setPlots(newPlots);

    const newInventory = (() => {
      const existing = inventory.find((item) => item.cropId === crop.id);
      if (existing) {
        return inventory.map((item) =>
          item.cropId === crop.id ? { ...item, count: item.count + 1 } : item
        );
      }
      return [...inventory, { cropId: crop.id, count: 1 }];
    })();
    ...
    if (hasHarvestDouble) {
      reward *= 2;
      newActiveBuffs = newActiveBuffs.filter(b => b.effect !== "harvest_double");
      setActiveBuffs(newActiveBuffs);
      showNotification(`✨ 幸运手套触发！收获 ${crop.emoji} x2！+${reward} 金币`);
    } else {
      showNotification(`✨ 收获了 ${crop.emoji} ${crop.name}！+${reward} 金币`);
    }

    const hasGoldBonus = newActiveBuffs.some(b => b.effect === "gold_bonus");
    if (hasGoldBonus && Math.random() > 0.5) {
      reward += 5;
      showNotification(`💰 黄金锄头触发！额外+5金币`);
      newActiveBuffs = newActiveBuffs.filter(b => b.effect !== "gold_bonus");
      setActiveBuffs(newActiveBuffs);
    }

    const newCoins = coins + reward;
    setCoins(newCoins);
    const expResult = addExp(20);
  ...
  const sellCrop = (cropId: string) => {
    ...
    const newCoins = coins + totalValue;
    setCoins(newCoins);
    const newInventory = inventory.filter((i) => i.cropId !== cropId);
    setInventory(newInventory);
    const expResult = addExp(item.count * 15);
    saveState({
      coins: newCoins,
      inventory: newInventory,
      exp: expResult.exp,
  ...
  const synthesize = () => {
    ...
    setInventory(newInventory);
    saveState({ inventory: newInventory });
    ...
    setTimeout(() => {
      ...
      const newSeedInventory = (() => {
        const existing = seedInventory.find(item => item.cropId === result.id);
        if (existing) {
          return seedInventory.map(item =>
            item.cropId === result.id ? { ...item, count: item.count + 1 } : item
          );
        }
        return [...seedInventory, { cropId: result.id, count: 1 }];
      })();
      setSeedInventory(newSeedInventory);
      saveStateRef.current({ seedInventory: newSeedInventory });
  ...
  const refreshShop = () => {
    ...
    saveState({ coins: newCoins, refreshCount: newRefreshCount });
  ...
  const buyShopItem = (offer: ShopOffer) => {
    ...
    const newItemInventory = [...itemInventory, offer.item];
    setItemInventory(newItemInventory);
    setShopOffers(prev => prev.filter(o => o.id !== offer.id));

    saveState({ inventory: newInventory, itemInventory: newItemInventory });
  ...
  const useItem = (item: Item) => {
    switch (item.effect) {
      case "instant_harvest":
        ...
        const newInstantItemInventory = itemInventory.filter(i => i !== item);
        setItemInventory(newInstantItemInventory);
        saveState({ itemInventory: newInstantItemInventory });
        break;
      case "random_seed":
        const randomCrop = RARE_CROPS[Math.floor(Math.random() * RARE_CROPS.length)];
        const newSeedInventory = (() => {
          const existing = seedInventory.find(i => i.cropId === randomCrop.id);
          if (existing) {
            return seedInventory.map(i => i.cropId === randomCrop.id ? { ...i, count: i.count + 1 } : i);
          }
          return [...seedInventory, { cropId: randomCrop.id, count: 1 }];
        })();
        setSeedInventory(newSeedInventory);
        const newRandomItemInventory = itemInventory.filter(i => i !== item);
        setItemInventory(newRandomItemInventory);
        saveState({ seedInventory: newSeedInventory, itemInventory: newRandomItemInventory });
      ...
      case "gold_bonus":
        ...
        const newActiveBuffs = [...activeBuffs, { effect: item.effect, endsAt }];
        setActiveBuffs(newActiveBuffs);
        const newBuffItemInventory = itemInventory.filter(i => i !== item);
        setItemInventory(newBuffItemInventory);
        let newMultiplier = growthBoostMultiplier;
        ...
        saveState({
          activeBuffs: newActiveBuffs,
          itemInventory: newBuffItemInventory,
          growthBoostMultiplier: newMultiplier,
        });
        break;
    }
  };
